    ...
    return userComments.length > 0 ? (
            <ul className="list-group p-0">
                {userComments.map(comment => <li
                        ...
                        key={comment.created}>
                        {comment.comment}
                        <span className="badge bg-primary rounded-pill delete-comment" onClick={() => deleteComment(comment.created, dispatch)}><i className='material-icons mx-2'>delete</i></span>
                    </li>
                )}
            </ul>
        )
